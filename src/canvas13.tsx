...
const CanvasComponent = () => {
  ...
  useEffect(() => {
    ...
    // Function to draw a straight L-shaped line between two points
    const drawStraightLine = (x1, y1, x2, y2) => {
      context.beginPath();
      context.moveTo(x1, y1);
      context.lineTo(x1, y2); // Horizontal part of the L-shape
    ...
    // Function to recursively draw the binary tree with straight-line connections
    const drawTree = (depth, x, y, horizontalGap) => {
      ...
      if (depth > 1) {
        drawStraightLine(x, y, x - currentGap, y + verticalGap);
    ...
    drawTree(treeDepth, canvas.width / 2, rootY, canvas.width / 4);
