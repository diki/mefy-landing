...
const CanvasComponent = () => {
  ...
  useEffect(() => {
    ...
    // Function to generate random tree with less dense leaf nodes
    const generateRandomTree = (node, depth, maxDepth) => {
      ...
      for (let i = 0; i < numChildren; i++) {
        ...
        generateRandomTree(
          childNode,
          depth + 1,
          maxDepth - Math.floor(Math.random() * 2)
        ); // Reduce density
      }
    };
    ...
    const drawTree = (node, x, y, width, level) => {
      ...
      let childX = x + horizontalGap;

      // Draw connections to children
      for (const child of node.children) {
        drawLine(x, y, childX, y + 80 + Math.random() * 50); // Vertical connection with randomness
        drawTree(
          child,
          childX,
          y + 80 + Math.random() * 50,
    ...
    // Start drawing the tree from the root (middle left of the screen)
    drawTree(rootNode, canvas.width * 0.1, 100, canvas.width * 0.8, 0);
